...
def make_hud(width, values, origin=(5, 4), complete=True):
    height = 32
    yy, xx = np.mgrid[:height, :width]
    image = (18 + (xx + 2 * yy) % 13).astype(np.uint8)
    x, y = origin
...
def check_case(width, values, origin):
    image = make_hud(width, values, origin)
...
def main():
    ...
    only_f = make_hud(48, {"F": 0xCAFE}, origin=(3, 6), complete=False)
    ...
    print("HUD OCR proof: OK (H32/H40 contiguous 22-cell layout, standalone F compatible)")
