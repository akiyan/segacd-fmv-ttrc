...
SRC_FRAME  = (1308, 52, 1877, 336)    # 右1段: Source
CATLEG_XY  = (1308, 348); CATLEG_W, CATLEG_H = 569, 44   # 凡例リスト(2行)= Categoryの「上」へ移動
CAT_FRAME  = (1308, 398, 1877, 648)   # Categoryパネル
CATTOT_XY  = (1308, 660); CATTOT_W, CATTOT_H = 569, 40   # カテゴリ合計(2行)= Category下(上の凡例一覧と同じ12px margin)
VRAM_FRAME = (1308, 734, 1877, 1066)  # 新パネル: VRAM(見出しは枠の「外(上)」の余白へ, 小フォント)
...
def draw_vram_placeholder(w, h):
    """VRAMパネルのプレースホルダ: 64KB VRAM=2048タイル(32B/タイル)を 64x32 グリッドで、
    実MDのVRAMアドレス順(左→右, 上→下。1行=64タイル=0x800)に配置。
    パターンタイル(0x20-)は適用予定CRAMパレットで着色, ネームテーブル(0xC000-0xFFFF)等の
    非パターン領域はグレースケール。実装は render_analysis 側(実VRAM状態を反映)。"""
    im = Image.new("RGB", (w, h), (10, 10, 12))
    d = ImageDraw.Draw(im)
    cols, rows = 64, 32                                  # 2048タイル(64KB/32B)
    rng = random.Random(7)
    for r in range(rows):
        for c in range(cols):
            idx = r * cols + c                           # = VRAMアドレス/32
            x0, y0 = int(c * w / cols), int(r * h / rows)
            x1, y1 = int((c + 1) * w / cols) - 1, int((r + 1) * h / rows) - 1
            if idx == 0:                                 # tile0(予約)
                col = (20, 20, 24)
            elif idx <= 1400:                            # パターンタイル: 適用CRAMで着色。
                base = (rng.randint(40, 230), rng.randint(40, 230), rng.randint(40, 230))
                # 現在表示/将来表示=フル, 未参照(キャッシュ済み)=パレットそのまま半分薄く
                col = base if rng.random() < 0.45 else tuple(v // 2 for v in base)
            elif idx < 1536:                             # パターン領域とNTの隙間=未使用(暗)
                col = (26, 26, 30)
            else:                                        # ネームテーブル 0xC000-0xFFFF=グレースケール
                g = rng.randint(40, 170); col = (g, g, g)
...
def main():
    ...
    panel(d, VRAM_FRAME)         # VRAM枠。見出しは枠の外(上)に小フォントで(下記)
    d.text((VRAM_FRAME[0] + 2, VRAM_FRAME[1] - 7), "VRAM  (64KB tilemap, palette applied)",
           fill=COL_TXT, font=f_lbl, anchor="ls")   # 小さめ(20)・cattotalsとVRAM枠の間の余白へ
    ...
    vv = draw_vram_placeholder(VRAM_FRAME[2] - VRAM_FRAME[0] - 2 * PAD, VRAM_FRAME[3] - VRAM_FRAME[1] - 2 * PAD)
    cv.paste(vv, (VRAM_FRAME[0] + PAD, VRAM_FRAME[1] + PAD))
