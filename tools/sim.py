...
def main():
    ...
    for i in range(n):
        ...
        def append_coa_bucket(key, bucket):
            coa_bucket[bucket].append(key)
        ...
        if MIDFAR_ON:
            ...
            def best_resident(c):
                ...
                b = (int(mbk[c, 0]), int(mbk[c, 1]), int(mbk[c, 2]))
                entry = resident_bucket_cache.get(b)
                if entry is None:
                    if _loop_profile:
                    ...
                    segments = pat_seg
                    for ck in reversed(coa_bucket[b]):
                        if (ck not in resident and ck not in loaded) or ck not in cached:
                            continue
                        if segments.get(ck) != cur_seg:  # index列が新CRAMでゴミ化する区間跨ぎは禁止
                            continue
                        cand.append(ck); cnt += 1
                        if cnt >= COA_K:
                            break
                    cand = tuple(cand)
                    arr = np.stack([cached[k] for k in cand]) if cand else None
                    entry = (cand, arr)
                    resident_bucket_cache[b] = entry
                else:
                    if _loop_profile:
    ...
    if _SLOT_LOCALITY_STAGE == "final":
        ...
        run_groups = _source_run_groups(replay, supply_sources_log)
        ...
        for frame_index, (
            ...
            slots.extend(sorted(
                int(final_mapping[slot]) for slot in prefetch_slots))
            sources.extend(
