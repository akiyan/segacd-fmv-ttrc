...
| exact late run-control charge | 4 bytes per realized source-aware run | sim | Run fragmentation is known only after the complete frame is placed, so future-reserve decisions fund update entries and Prg payload without charging one worst-case descriptor per cold tile. After allocation they charge the exact run count; that exact lower allowance enters the next frame and automatically repays any temporary reserve undershoot. A separate hard-solvency check still applies the one-descriptor-per-cold upper bound only when needed to prevent the current frame from selecting more total bytes than physically exist. It does not retain allowance for a future frame. The finalized slot-map replay also rejects any frame whose exact update, payload, and run work is unfunded. |
