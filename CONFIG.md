...
| output dither | on by default | Position-fixed Bayer dithering before palette mapping. Set `[encoder].dither=false` only when the source already carries visible dithering and a second pass creates isolated palette-extreme pixels. |
| segmented palettes / Near / boot VRAM prefetch | on | Fixed encoder behavior, not profile settings. |
...
resource. It does not feed a smaller local Prg/cold cap back into the encoder,
repeat allocation with a new cap, or alter the source's qualified cold cap.
This keeps cold quality selection separate from physical delivery diagnosis.
...
normal ceiling: `0180` at 15fps, `018F` at 24fps, or `0194` at 30fps). An
...
Schema v3 removes hardware-sized encoder/palette keys and the entire `[pack]`
table. Source-dependent output dithering remains an optional encoder setting.
