...
| `FEATURE_COLD_RUNS` | header bit 0 at offset 62 | pack / sp | Appends `(slot_start,count)` cold-run descriptors after each aligned audio chunk. The encoder freezes logical decisions in a seed pass, accounts for that map's real run cost in a second pass, then derives the delivered movie-wide logical-to-physical slot permutation from those completed decisions. Cold payloads use ascending physical-slot order; name updates remain in cell order. At 24fps or above, the Sub copies eligible blocks by these runs instead of scanning every update entry again. Old streams use the entry fallback; old players ignore the suffix via `total_len`. |
