...
| `FEATURE_COLD_RUNS` | header bit 0 at offset 62 | pack / sp | Appends `(slot_start,count)` cold-run descriptors after each aligned audio chunk. The encoder applies a fixed movie-wide logical-to-physical slot permutation and emits cold payloads in ascending physical-slot order; name updates remain in cell order. At 24fps or above, the Sub copies eligible blocks by these runs instead of scanning every update entry again. Old streams use the entry fallback; old players ignore the suffix via `total_len`. |
...
| `RUN_TABLE` | 1536 records by address range; current cold cap is much lower | ip | `(dst, len, src)` table of contiguous physical cold-slot runs. Each record is counted by HUD `N`; a one- or two-tile record uses CPU writes, while a longer record can become one or more DMA commands at VBlank boundaries. The encoder minimizes worst-frame cold plus run-boundary cost, not whole-movie run total, so light frames may gain runs when that removes a deadline cliff. |
...
| `[encoder]` | `gpu`, `vram_tiles`, `dither`, `segment_palettes`, `near`, `coa`, `raw_prefetch` | Common codec controls. `raw_prefetch` defaults to false. When enabled, the encoder may spend spare BODY/cold capacity to place exact patterns needed by the next frame when its protected cold demand exceeds the measured cap. There is no fixed current-cold threshold: the exact remaining cold and BODY allowances are authoritative. A speculative batch needs room for at least four patterns and is capped at 32. It uses only otherwise safe VRAM slots; visible work always takes priority and may reclaim a speculative slot. GPU is the default; CPU fallback remains automatic. BODY supply comes from the exact CD-1x sector cadence after reserving control data and is not configurable per source. |
