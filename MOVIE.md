...
VERSION        = 1               (bump when the header or a block layout changes)
...
+--------------------------------------------------+
| FRAME 0  (FRAME_SECTORS = 5 sectors)             |
| FRAME 1  (5 sectors)                             |
| ...                                              |
...
| Off | Size | Field        | Meaning |
|-----|------|--------------|---------|
| 22  | 4    | prebuf_pat   | `Bpat`: number of cold patterns pre-buffered before frame 0 |
...
(cold tile patterns) and how many are control. The Sub CPU uses this to split
the continuous read into payload (DMA to the PRG-RAM ring) and control (to the
apply buffer). `n_pay_sec + n_ctrl_sec <= 5`; the rest is padding.

## Prebuffer

`prebuf_sec` sectors holding the first `Bpat` cold patterns (32 bytes each),
loaded into the ring before playback so early frames are not starved.

## Frame (exactly FRAME_SECTORS = 5 sectors)
...
**fixed position right after the 6-byte mini-header**, so a player reads it at a
...
- bits 0..14 = the ring **slot** index. The VDP name-table entry for the cell is
  set to tile `base + slot`. This is the core *tile texture reuse*: most cells
  cost just this 2-byte entry.

**Audio**: 887 bytes/frame of RF5C164 sign-magnitude 8-bit PCM (positive =
`0..0x7F`, negative = `0x80 | magnitude`), fed to the PCM chip in sync.
